...
describe('Comprehensive property sorting across all strategies', () => {
    ...
    it('should correctly sort all properties using grouped strategy', () => {
        ...
        // Box model properties should be grouped (allow for a wider margin because of how properties are grouped)
        expect(Math.abs(displayIndex - result.sortedProperties!.indexOf('flex'))).toBeLessThanOrEqual(15);
        expect(Math.abs(displayIndex - result.sortedProperties!.indexOf('gap'))).toBeLessThanOrEqual(15);

        // Logical properties should be grouped with their physical counterparts
        expect(Math.abs(
    ...
    it('should correctly sort all properties using alphabetical strategy', () => {
        ...
        // All transform related properties should be adjacent
        expect(Math.abs(webkitTransformIndex - transformIndex)).toBeLessThanOrEqual(2);
        expect(Math.abs(mozTransformIndex - transformIndex)).toBeLessThanOrEqual(2);            // Check the resulting array contains all the original properties
    ...
    it('should correctly sort all properties using concentric strategy', () => {
        ...
        // Positioning should come before margin
        ...
        expect(Math.abs(colorIndex - result.sortedProperties!.indexOf('background-color'))).toBeLessThanOrEqual(10);

        // Aspect ratio should be with dimensions
        expect(Math.abs(
            result.sortedProperties!.indexOf('width') -
            result.sortedProperties!.indexOf('aspect-ratio')
        )).toBeLessThanOrEqual(10);            // Check the resulting array contains all the original properties (excluding empty lines)
    ...
    it('should correctly sort all properties using idiomatic strategy', () => {
        ...
        // Position should come before display/box model
        expect(positionIndex).toBeLessThan(displayIndex);

        // Display should come before margin
        expect(displayIndex).toBeLessThan(marginIndex);

        // Width should be in box sizing section
        expect(Math.abs(widthIndex - result.sortedProperties!.indexOf('height'))).toBeLessThanOrEqual(5);

        // Aspect ratio should be with other dimension properties
        expect(Math.abs(
            result.sortedProperties!.indexOf('width') -
            result.sortedProperties!.indexOf('aspect-ratio')
        )).toBeLessThanOrEqual(7); // Increased to 7 to account for the empty line after CSS variables

        // Container properties should be near the top
        expect(result.sortedProperties!.indexOf('container')).toBeLessThan(colorIndex);
